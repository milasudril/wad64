...
Wad64::Archive::Archive(FileReference ref):m_file_ref{ref}
{
	WadInfo info;
	errno = 0;
	auto const n_read = m_file_ref.read(std::span{reinterpret_cast<std::byte*>(&info), sizeof(info)});
	...
	std::generate_n(std::inserter(m_directory, std::end(m_directory)), info.numlumps, [src = m_file_ref]()
	{
		FileLump lump;
		auto const n_read = src.read(std::span{reinterpret_cast<std::byte*>(&lump), sizeof(lump)});
		...
		return std::pair{std::u8string{std::data(lump.name)}, DirEntry{lump.filepos, lump.size}};
	});
}