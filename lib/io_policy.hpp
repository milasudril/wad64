...
namespace Wad64
{
	...
	public:
		...
		size_t read(std::span<std::byte> buffer) const
		{ return m_read(m_ref, buffer); }

		size_t write(std::span<std::byte const> buffer) const
		{ return m_write(m_ref, buffer); }

		int64_t seek(int64_t offset) const
		{ return m_seek(m_ref, offset); }
