...
void Wad64Cli::Remove::operator()() const
{
	...
	{
		case Mode::Secure:
			std::ranges::for_each(items, [prefix = Wad64::BeginsWith{m_path.entryPrefix().c_str()},
				&archive](auto const& item) {
				if(item.first == prefix)
				{ archive.secureRemove(item.first); }
			});
			break;
...
std::unique_ptr<Wad64Cli::Command> Wad64Cli::Remove::create(int argc, char const* const* argv)
{
	if(argc == 1)
	{
		return std::make_unique<Remove>(ArchivePath::parse(argv[0]), Mode::Fast);
	}
